...
$$
Y_{ijkl} = \mu+ \alpha + \beta + \gamma\ + \delta\ + (*\gamma\delta){ij} + \epsilon_k\ + \theta_l\ + (\epsilon\theta)_{kl} + (\delta\epsilon\theta)_{jkl} + e_{ijkl} 
$$
...
1: Does light have an effect on quality? $$H_0: \alpha_{i} = 0 \ ,i = 1, 2, 3, 4$$
$$H_1: at~least~one~\alpha_{i} \ne 0 \ , i = 1, 2, 3, 4$$

2: Does heat have an effect on quality? $$H_0: \beta_{j} = 0 \ ,j = 1, 2, 3, 4$$
$$H_1: at~least~one~\beta_{j} \ne 0 \ , j = 1, 2, 3, 4$$

3: Does the effect of light depend on the level of heat? $$H_0: (\alpha\beta)_{ij} = 0 \ ,i,j = 1, 2, 3, 4$$
$$H_1: at~least~one~(\alpha\beta)_{ij} \ne 0 \ , i,j = 1, 2, 3, 4$$

4: Does the effect of light and heat depend on variety? $$H_0: (\alpha\beta\gamma)_{ijk} = (\alpha\gamma)_{ik} = (\beta\gamma)_{jk} = 0 \ ,i,j = 1, 2, 3, 4 \ , k = R, F$$
...
## Does the optimal heat and ligth settings depend on variety?

```{r}

library(emmeans)

emmeans(model, ~ light + variety)

emmeans(model, ~ heat + variety)
