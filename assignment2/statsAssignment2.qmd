...
model$coefficients[22:24] |> signif(4)

contrast(emm_l, 
         method = "del.eff", by = NULL)

```

Our analysis provides evidence for the interaction between light and variety having an effect on quality (p-val < 0.005). In order to further test this result, we conducted a t-test involving contrasts.
...
contrast(emm_l, interaction=TRUE, by=NULL)

#contrast(emm_h, list(avg_R_minus_avg_F = c(rep(1/4, 4), rep(-1/4, 4))), by = NULL )
contrast(emm_h, interaction=TRUE, by=NULL)
