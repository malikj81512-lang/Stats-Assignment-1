...
The unstratified histogram does not mimic the shape of the overlaid normal curve which suggests that some households consume less electricity and some households consume more, hence the two peaks of the histogram.
...
When the histogram is stratified with solar installation being true, the histogram follows the normal curve more closely which suggests that solar installation results in lower mean consumption and a more left skewed distribution. When the histogram is stratified with solar installation being false, the histogram follows the normal density curve more closely with a right skewed distribution showing higher mean consumption than when solar is installed. Clearly, stratifying the histogram of consumption with solar installation gives a clearer picture of the effect of solar on consumption and explains the shape of the unstratified histogram.
...
pairs(data_tidy_energy_use[1:6], 
      labels = c("Consumption (kWh)", "Outside Temperature", "Humidity", "Wind Speed", "Household Size", "Appliance Index"), col = "blue4")
...
3.  Outliers: There are two apparent outlying observations of low consumption on Saturdays as well as an outlying observation of high consumption in a household without solar installed.
...
results2 <- tibble(
  ...
  value = c(residual_std_error, r_squared, adj_r_squared, f_stat) |> signif(4),
...
summary(lm(consumption_kwh~energy_efficiency, data = en_dat))
...
b_hat <- lm(consumption_kwh ~ outside_temperature + humidity + wind_speed + household_size + appliance_index + energy_efficiency + solar_installed + day_of_week + holiday + outside_temperature:humidity, data_tidy_energy_use)
...
# consumption_kw ~ outside_temperature
# Test at 5% significance 

p_val_out_temp <- sum$coefficients[2, 4]
p_val_out_temp
# p_val_out_temp > 0.05 -> do not reject

# consumption_kw ~ humidity 
# Test at 5% significance

p_val_out_hum <- sum$coefficients[3, 4]
p_val_out_hum
# p_val_out_hum > 0.05 -> do not reject

# size of household?
#summary(data_tidy_energy_use$consumption_kwh)
#summary(data_tidy_energy_use$household_size)
lm_household <- (lm(consumption_kwh~outside_temperature + humidity + wind_speed + appliance_index + energy_efficiency + solar_installed + day_of_week + holiday + outside_temperature:humidity, data_tidy_energy_use))
anova(lm_household, b_hat)
...
cat("\nEstimated power for β1 =", b1, "is", power_est[j], "\n")
}
...
for (i in 1:n_sim){
  ...
  model <- lm(y ~ x_vec)
  p_val <- summary(model)$coefficients[2, 4]
  rejections_0.5[i] <- as.integer(p_val < 0.05)
}

power_est_0.5 <- mean(rejections_0.5)
cat("\nEstimated power for β1 =", b1, "is", power_est_0.5, "\n")

# True b1 = 1
b1 <- 1
rejections_1 <- numeric(n_sim)
...
for (i in 1:n_sim) {
  ...
  model <- lm(y ~ x_vec)
  p_val <- summary(model)$coefficients[2, 4]
  rejections_1[i] <- as.integer(p_val < 0.05)
}

power_est_1 <- mean(rejections_1)
cat("Estimated power for β1 =", b1, "is", power_est_1, "\n")

#True b1 = 2
b1 <- 2
rejections_2 <- numeric(n_sim)
...
for (i in 1:n_sim) {
  ...
  model <- lm(y ~ x_vec)
  p_val <- summary(model)$coefficients[2, 4]
  rejections_2[i] <- as.integer(p_val < 0.05)
}

power_est_2 <- mean(rejections_2)
cat("Estimated power for β1 =", b1, "is", power_est_2, "\n")


#True b1 = 5
b1 <- 5
rejections_5 <- numeric(n_sim)
...
for (i in 1:n_sim) {
  ...
  model <- lm(y ~ x_vec)
  p_val <- summary(model)$coefficients[2, 4]
  rejections_5[i] <- as.integer(p_val < 0.05)
}

power_est_5 <- mean(rejections_5)
cat("Estimated power for β1 =", b1, "is", power_est_5, "\n")

#True b1 = 10
b1 <- 10
rejections_10 <- numeric(n_sim)
...
for (i in 1:n_sim) {
  ...
  model <- lm(y ~ x_vec)
  p_val <- summary(model)$coefficients[2, 4]
  rejections_10[i] <- as.integer(p_val < 0.05)
}

power_est_10 <- mean(rejections_10)
cat("Estimated power for β1 =", b1, "is", power_est_10, "\n")
...
cat("Estimated power for n  =", n, ", assuming incorrect linear model = ", est_power_lin[j], "\n")
...
cat("Estimated power for n  =", n, ", assuming correct exponential model = ", est_power_exp[j], "\n")
