...
# adding a legend 
legend("topleft", 
  legend = c("Histogram Density", "Normal Density"),
       fill = c("black", "red3"))
...
```
 ...
 When the histogram is stratified with solar installation being true, the histogram follows the normal curve more closely which suggests that solar installation results in lower mean consumption and a more left skewed distribution. 

### 2. Pairwise plots

```{r Pairs, echo = FALSE}
#| fig.cap: "Pairwise plots of continuous variables (consumption, outside temperature, humidity and wind speed) against each other"
pairs(data_tidy_energy_use[1:4],
      labels = c("Consumption", "Outside Temperature", "Humidity", "Wind Speed")
)
